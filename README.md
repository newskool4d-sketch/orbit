...
- **오늘:** 다음 일정, 직접 등록한 중요 날짜(D-day), 처리할 할 일, 최근 파일, Codex·Claude Code 각 최신 작업.
...
- **설정:** 테마, 글자 크기, 고정, 파일 폴더, Google 로그인, 소스별 연결/오류 상태.

중요 날짜는 Google 연결 없이 제목과 날짜를 등록할 수 있습니다. 오늘 화면에는 중요 표시와 날짜순으로 최대 3개가 보이며, 전체 보기에서 편집·보관·복원·삭제합니다. 데이터는 각 기기의 Orbit 앱 데이터 폴더에만 저장되고 다른 기기나 Google과 동기화되지 않습니다.
